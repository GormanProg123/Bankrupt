...
const SavingsGoals = () => {
  const modalState = useSelector((state:RootState) => state.savingsGoalModalSlice.modalState)
    ...
    <aside className="saving-goals border rounded-lg p-4 md:p-5 bg-white shadow-sm">
      ...
      {savingGoals.map((goal, id) => (
          <div className="mb-5" key={id}>
            ...
            <p className="text-xs md:text-sm text-gray-500">Target: ${goal.goal}</p>
            <div className="w-full h-2 bg-gray-200 rounded mt-2 mb-1">  
              <div className="h-full bg-black rounded" style={{ width: `${Number(((goal.balance / goal.goal) * 100).toFixed(2)) > 100 ? 100 : ((goal.balance / goal.goal) * 100).toFixed(2)}%` }}></div>
            </div>
      ...
      <button onClick={() => dispatch(savingsModalState({modalState:!modalState}))} className="w-full  bg-black text-white py-2 md:py-3 rounded font-semibold hover:opacity-90 cursor-pointer text-sm md:text-base">
        + Add New Goal
      </button>
